...
raw_data <- read_csv("AAKANDlT.csv") #need to change this name!
...
## Creating gender groups, age groups, education levels

#Filter out subjects under 18
reduced_census_data <- reduced_census_data %>% 
  filter(age >= 18)

#Re-categorizing gender/sex?
reduced_census_data$sex <- as.factor(reduced_census_data$sex)

#Age groups
reduced_census_data$age_group = case_when(
  reduced_census_data$age >= 18 & reduced_census_data$age < 30 ~ "18 - 29",
  reduced_census_data$age >= 30 & reduced_census_data$age < 45 ~ "30 - 44",
  reduced_census_data$age >= 45 & reduced_census_data$age < 60 ~ "45 - 59",
  reduced_census_data$age >= 60 ~ "60+")

reduced_census_data$age_group <- as.factor(reduced_census_data$age_group)

#Highest education levels
reduced_census_data$education_group = case_when(
  reduced_census_data$education == "Less than high school diploma or its equivalent" ~ "Less than high school",
  reduced_census_data$education == "High school diploma or a high school equivalency certificate" |
    reduced_census_data$education == "Trade certificate or diploma" ~ "High school graduate",
  reduced_census_data$education == "College, CEGEP or other non-university certificate or di..." |
    reduced_census_data$education == "University certificate or diploma below the bachelor's level" ~ "Some postsecondary education",
  reduced_census_data$education == "University certificate, diploma or degree above the bach..." | 
    reduced_census_data$education == "Bachelor's degree (e.g. B.A., B.Sc., LL.B.)" ~ "University graduate")

reduced_census_data$education_group <- as.factor(reduced_census_data$education_group)

#Creating the cells
reduced_census_data <- 
  reduced_census_data %>%
  count(age_group, sex, province, education_group) %>%
  group_by(age_group, sex, province, education_group) 

# Saving the census data as a csv file in working directory
write_csv(reduced_census_data, "censusdata.csv")
